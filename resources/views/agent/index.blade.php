            ...
            <table id="datatable-buttons" class="table table-light table-bordered">
              ...
              <tbody style="color: #73879C">
                ...
                <tr>
                  ...
                  <td>{{$item->nama}}</td>
                  <td>{{$item->nomor_kantor}}</td>
				  <td>{{$item->user->email ?? '--'}}</td>
                  <td>{{$item->pic}}</td>
                  <td>{{$item->hp}}</td>
                  <td>
                    <a href="{{url('agent/'.$item->id.'/edit')}}" role="button" class="btn btn-success btn-sm">
                      <i class="fa fa-pen"></i>Edit
                    </a>
                    <form action="{{url('agent/'.$item->id)}}" method="post" onsubmit="return confirm('Yakin hapus data?')" style="display:inline">
                      @method('delete')
                      @csrf
